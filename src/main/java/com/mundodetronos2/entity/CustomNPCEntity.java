...
public class CustomNPCEntity extends PathfinderMob {
    ...
    private String currentAnimationOverride = "";
    ...
    public void playAnimation(String animName) {
        if (animName != null && !animName.isEmpty()) {
            this.currentAnimationOverride = animName;
            this.animationTicks = 40; // 2 seconds override duration
        }
    }

    public void resetAnimation() {
        this.currentAnimationOverride = "";
        this.animationTicks = 0;
    }

    public String getActualCurrentAnimation() {
        if (this.animationTicks > 0 && !this.currentAnimationOverride.isEmpty()) {
            return this.currentAnimationOverride;
        }

        double speedSq = this.getDeltaMovement().x() * this.getDeltaMovement().x() + this.getDeltaMovement().z() * this.getDeltaMovement().z();
        if (speedSq > 0.001D) {
    ...
    public net.minecraft.world.InteractionResult mobInteract(Player player, net.minecraft.world.InteractionHand hand) {
        if (!player.level().isClientSide() && player instanceof net.minecraft.server.level.ServerPlayer sp) {
            ...
            playAnimation(getInteractionAnimation());

            // Right click on Ivan opens Guild GUI directly!
            if ("ivan".equalsIgnoreCase(type) || name.toLowerCase().contains("ivan") || name.toLowerCase().contains("karla")) {
            ...
            com.mundodetronos2.network.MessageManager.actionBar(sp, "§a" + name + ": ¡Hola, viajero!");
            return net.minecraft.world.InteractionResult.sidedSuccess(player.level().isClientSide());
        }
