...
import com.mundodetronos2.block.CrateBlockEntity;
...
public class CrateBlockEntityRenderer implements BlockEntityRenderer<CrateBlockEntity> {
    ...
    @Override
    public void render(CrateBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BlockState state = blockEntity.getBlockState();
        int crateLvl = 1;
        if (state.hasProperty(CrateBlock.LEVEL)) {
        ...
        if (entry == null || entry.model == null) {
            entry = BlockModelRegistry.get("lvl1_crate");
        }
