...
public class ThroneBlockEntityRenderer implements BlockEntityRenderer<ThroneBlockEntity> {

    public ThroneBlockEntityRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(ThroneBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BlockPos pos = blockEntity.getBlockPos();
        ThroneData throne = ThroneManager.getThroneAt(pos);

        String modelId = "sand_castle";
        if (throne != null && throne.getState() == ThroneState.REPAIRING) {
            modelId = "sand_castle_parts";
        }

        BlockModelRegistry.Entry entry = BlockModelRegistry.get(modelId);
        if (entry == null || entry.model == null) {
            entry = BlockModelRegistry.get("sand_castle");
        }
